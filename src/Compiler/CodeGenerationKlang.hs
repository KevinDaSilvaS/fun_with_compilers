...
makeFile ir = do
    writeFile "./klang_parsed.js" (startCodeGen ir)
    return ()
...
generateCodeFromExpr (Value (IdentifierToken, "readLine") expr) = valueVariable
    where
        valueVariable = " prompt(\"Please enter a valid string\") " 
            ++ generateCodeFromExpr expr
generateCodeFromExpr (Value (IdentifierToken, "readNum") expr) = valueVariable
    where
